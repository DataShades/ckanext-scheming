...
def get_blueprints() -> list[Blueprint]:
    dataset_bp = Blueprint(DATASET_BP, dataset.import_name, url_prefix="/<package_type>")
    add_paged_form_rules(dataset_bp)
    register_dataset_plugin_rules(dataset_bp)
    dataset_bp.before_request(_dynamic_type_or_404)

    resource_bp = Blueprint(RESOURCE_BP, resource.import_name, url_prefix="/<package_type>/<id>/resource")
...
def build_dynamic_type_url(
    ...
    if name.endswith("_resource"):
        ...
        if _dynamic_schema_exists(package_type):
            return flask_url_for(f"{RESOURCE_BP}.{view}", **{**values, "package_type": package_type})
...
def _dynamic_type_or_404() -> Response | None:
    view_args = tk.request.view_args or {}
    package_type = view_args.get("package_type")

    if not package_type or not SchemingSchemaVersion.head_version("dataset", package_type):
        try:
            return current_app.send_static_file(tk.request.path.lstrip("/"))
        except NotFound:
